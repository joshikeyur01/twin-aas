"""Entrypoint: the pose loop and the health server as sibling tasks.
...
from feeder.health import build_health_app
...
async def main() -> None:
    configure_logging()
    config = FeederConfig.from_env()
    feeder = PoseFeeder(config)
    app = build_health_app("feeder", feeder.readiness)
    ...
    async with asyncio.TaskGroup() as tg:
        tg.create_task(feeder.run(), name="pose")
